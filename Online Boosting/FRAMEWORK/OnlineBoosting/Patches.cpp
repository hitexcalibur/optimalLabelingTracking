...
Patches::Patches(SampleSet sampleSet)
{
	ROI = sampleSet.getROI();
	this->num = sampleSet.size();
	this->numPatchesX = sampleSet.getSampleX();
	this->numPatchesY = sampleSet.getSampleY();
	patches = new Rect[num];
	for (int i = 0; i < this->num; i++)
	{
		patches[i].left = sampleSet[i]._col;
		patches[i].upper = sampleSet[i]._row;
		patches[i].height = sampleSet[i]._height;
		patches[i].width = sampleSet[i]._width;
...
PatchesRegularScan::PatchesRegularScan(SampleSet sampleSet)
{
	ROI = sampleSet.getROI();
	num = sampleSet.size();
	numPatchesX = sampleSet.getSampleX();
	numPatchesY = sampleSet.getSampleY();
	patches = new Rect[num];
	for (int i = 0; i < this->num; i++)
	{
		patches[i].left = sampleSet[i]._col;
	...
	m_patchGrid = sampleSet.getpatchGrid();
	Size patchSize = Size(sampleSet[0]._height, sampleSet[0]._width);
