...
void BeyondSemiBoostingTracker::classify(ImageRepresentation* image, ImageRepresentation* BGMRep, Patches* patches)
{
	...
	if (m_confidences)
		delete[] m_confidences;
	m_confidences = new float[patches->getNum()];
...
void BeyondSemiBoostingTracker::myupdate(ImageRepresentation* image, ImageRepresentation* BGMRep, Patches* patches, Rect tracked)
{
	...
	if (confidence > 0) trackedLost = false;
	else trackedLost = true;
	this->updateOn(image, BGMRep, patches);
