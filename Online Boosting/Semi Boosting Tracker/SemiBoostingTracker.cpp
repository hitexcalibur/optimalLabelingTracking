...
void SemiBoostingTracker::classify(ImageRepresentation* image, Patches* patches)
{
...
void SemiBoostingTracker::update(ImageRepresentation* image, Patches* patches, Rect tracked)
{
	float off;
...
bool SemiBoostingTracker::track(ImageRepresentation* image, Patches* patches)
{
	...
	if (detector->getNumDetections() <=0 )
	{
		confidence = 0;
		priorConfidence = 0;
		cout << "SemiBoostingTracker Lost" << endl;
		trackedLost = true;
		return false;
	}
