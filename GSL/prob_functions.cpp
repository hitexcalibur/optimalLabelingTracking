...
//#include <gsl/gsl_nan.h>//added by bnzhong
...
double computeLikelihood (Dataset *data)
{
	...
	for (j = 0; j < data->numImages; j++) {
		double P1 = data->priorZ1;
		double P0 = 1 - data->priorZ1;
		for (idx = 0; idx < data->numLabels; idx++) {
...
double computeQ (Dataset *data)
{
	...
	for (j = 0; j < data->numImages; j++) {
		Q += data->probZ1[j] * log(data->priorZ1);
		Q += data->probZ0[j] * log(1 - data->priorZ1);
	}
	...
	for (idx = 0; idx < data->numLabels; idx++) {
		...
		//if (logSigma == - INFINITY)//modified by bnzhong
		if (logSigma == GSL_NEGINF) 
		{
		...
		//if (logOneMinusSigma == - INFINITY)//modified by bnzhong
		if (logSigma == GSL_NEGINF) 
		{
...
void EStep (Dataset *data)
{
	...
	for (j = 0; j < data->numImages; j++) {
		data->probZ1[j] = log(data->priorZ1);
		data->probZ0[j] = log(1 - data->priorZ1);
	}
	...
	for (j = 0; j < data->numImages; j++) {
		...
		//if (isnan(data->probZ1[j]))//modified by bnzhong
		if (_isnan(data->probZ1[j]))
		{
			abort();
		}
	}
