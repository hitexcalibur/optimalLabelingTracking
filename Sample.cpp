...
void		SampleSet::sampleImage(Matrixu* img, int x, int y, int w, int h, float inrad, float outrad, int maxnum)
{
	...
	uint roiHeight = 2 * inrad + h;
	uint roiWidth = 2 * inrad + w;
	if (roiHeight > img->rows()) roiHeight = img->rows() - minrow;
	if (roiWidth > img->cols())  roiWidth = img->cols() - mincol;
	ROI = Rect(minrow, mincol, roiHeight, roiWidth);
	...
	int i=0;
	sampleX = sampleY = 0;
	patchGrid = Size((maxrow-minrow+1), (maxcol-mincol+1));
