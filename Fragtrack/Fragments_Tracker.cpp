							 ...
							 vector<int>& y_coords)
{
	...
	CvMat *combined_vote;
	combined_vote = cvCreateMat(maxrow-minrow+1,maxcol-mincol+1,CV_32F);
	...
	//added by excalibur; Put all values to m_confidences
	int patchNum = (maxrow-minrow+1) * (maxcol-mincol+1);
	if (m_confidences == NULL) 
	{
		m_confidences = new float[patchNum];
		confidencesNum = patchNum;
	}
	else if (confidencesNum = patchNum)
	{
	...
	for (int i = 0; i < maxrow-minrow+1; i++)
		...
		{
			float tempVal = cvGetReal2D(combined_vote, i, j);
			m_confidences[num++] = tempVal;
		}
	//add end

	cvMinMaxLoc(combined_vote,&minval,&maxval,&min_loc,&max_loc,NULL);
	resultIdx = min_loc.y * (maxcol - mincol + 1) + min_loc.x;
	int cx = min_loc.x;
	int cy = min_loc.y;
